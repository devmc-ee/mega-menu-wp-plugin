...
test('language tabs are switching', async ({ page }) => {
  ...
  expect(enButton).toHaveClass('devmcee-mega-menu-languages-tabs-tab devmcee-mega-menu-languages-tabs-tab--active');

  expect(etButton).toBeVisible();
  expect(etButton).toHaveText('et');
  await etButton.click({ force: true });
  expect(etButton).toHaveClass('devmcee-mega-menu-languages-tabs-tab devmcee-mega-menu-languages-tabs-tab--active');
});