...
test.describe( 'Admin: Sub Menu Items', () => {
	test( 'add, edit, delete sub-item', async ( { page } ) => {
		...
		await page
			.getByTestId( 'menu-item-form__title' )
			.scrollIntoViewIfNeeded();
		await expect(
			page.getByTestId( 'menu-item-form__title' )
		).toBeVisible();
		...
		await page
			.getByTestId( 'menu-item-form__url' )
			.fill( '/new-menu-item' );
		const saveMenuItemButton = page.getByTestId(
			'menu-item-form__save-button'
		);
		...
		// open edit form
		await page
			.getByTestId( `${ menuItemTitle }-edit-button` )
			.scrollIntoViewIfNeeded();
		await expect(
			page.getByTestId( `${ menuItemTitle }-edit-button` )
		).toBeEnabled();
		await expect(
			page.getByTestId( `${ menuItemTitle }-edit-button` )
		).toBeVisible();
		...
		const addSubItemButton = page.getByTestId(
			'sub-items-column-0__add-sub-item-button'
		);
		...
		// cancel form
		const formCancelButton = page.getByTestId(
			'sub-menu-item-form__cancel-button'
		);
		...
		await expect(
			page.getByTestId( 'sub-items-column-0__add-sub-item-button' )
		).toBeVisible();
		await page
			.getByTestId( 'sub-items-column-0__add-sub-item-button' )
			.click();
		await page.getByTestId( 'sub-menu-item-form' ).isVisible();
		await expect( page.getByTestId( 'sub-menu-item-form' ) ).toBeVisible();
		...
		await expect(
			page.getByTestId( 'sub-menu-item-form__title' )
		).toBeVisible();
		await expect(
			page.getByTestId( 'sub-menu-item-form__description' )
		).toBeVisible();
		await expect(
			page.getByTestId( 'sub-menu-item-form__url' )
		).toBeVisible();

		await page
			.getByTestId( 'sub-menu-item-form__title' )
			.fill( subMenuItemTitle );
		await page
			.getByTestId( 'sub-menu-item-form__description' )
			.fill( `description-${ Date.now() }` );
		await page
			.getByTestId( 'sub-menu-item-form__url' )
			.fill( `/new-menu-item/${ subMenuItemTitle }` );

		// save sub-menu item
		const saveSubMenuItemButton = page.getByTestId(
			'sub-menu-item-form__save-button'
		);
		...
		// edit sub-menu item
		const subMenuEditButton = page.getByTestId(
			`sub-menu-item-form-${ subMenuItemTitle }__edit-button`
		);
		...
		await page
			.getByTestId( 'sub-menu-item-form__title' )
			.fill( editedSubMenuItemTitle );
		await saveSubMenuItemButton.click();
		await expect( page.getByTestId( 'sub-menu-item-form' ) ).toBeHidden();

		// delete sub-menu item
		const subMenuEditedEditButton = page.getByTestId(
			`sub-menu-item-form-${ editedSubMenuItemTitle }__edit-button`
		);
		...
		await expect( page.getByTestId( 'sub-menu-item-form' ) ).toBeVisible();
		const deleteSubMenuItemButton = page.getByTestId(
			'sub-menu-item-form__delete-button'
		);
		...
		// delete menu item
		const deleteMenuItemButton = page.getByTestId(
			`${ menuItemTitle || 'menu-item-form' }__delete-button`
		);
