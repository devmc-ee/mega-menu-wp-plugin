...
test('open and delete menu item form', async ({ page }) => {
  await page.goto('/wp-admin/post.php?post=12&action=edit&lang=en');
  await page.getByTestId('add-menu-item-button').click({ force: true });
  await expect(page.locator('form.devmcee-mega-menu-builder-menu-item-form')).toBeVisible();
  // cancel form
  await expect(page.getByTestId('menu-item-form__delete-button')).toBeDisabled();
  await page.getByTestId('menu-item-form__cancel-button').click({ force: true });
  await expect(page.locator('form.devmcee-mega-menu-builder-menu-item-form')).toBeHidden();
})
...
test('add nem menu item, edit menu item, delete item', async ({ page }) => {
  ...
  expect(await page.getByTestId('menu-item-form__save-button').isDisabled()).toBeTruthy();

  await page.getByTestId('menu-item-form__title').fill(title);
  expect(await page.getByTestId('menu-item-form__title').inputValue()).toBe(title);

  expect(await page.getByTestId('menu-item-form__save-button').isDisabled()).toBeTruthy();

  await page.getByTestId('menu-item-form__url').fill('/new-menu-item');
  await expect(page.getByTestId('menu-item-form__save-button')).toBeEnabled();
  ...
  await expect(page.getByTestId('menu-item-form__cancel-button')).toBeVisible();
  await page.locator('_react=MenuItemForm').isVisible();

  // no changes
  expect(await page.getByTestId('menu-item-form__url').inputValue()).toBe('/new-menu-item');
  ...
  await expect(page.locator('form.devmcee-mega-menu-builder-menu-item-form')).toBeHidden();
  expect(await page.getByTestId(`${title}-edit-button`).count()).toBe(0);
