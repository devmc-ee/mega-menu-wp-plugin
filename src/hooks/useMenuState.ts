...
import {
	MenuDispatchContext,
	MenuStateContext,
} from '../providers/MenuDataContextProvider';
import {
	MenuItemForm,
	MenuStateActionType,
	SubMenuItemForm,
} from '../reducers/menuState.types';
import {
	LanguageCode,
	MenuItem,
	SubMenuItem,
...
export const useMenuState = () => {
	...
	const initMenuItemFormForEditing = (
	...
	const removeMenuItemForm = () =>
		dispatch({ type: MenuStateActionType.MENU_ITEM_FORM_REMOVED });
	...
	const deleteMenuItemUuidFromLocalMenu = (
		uuid: UUID,
		locale: LanguageCode
	) => {
		dispatch({
	...
	const changeMenuItemFieldValue = (
		fieldName: keyof MenuItemForm,
		value: ValueOf<MenuItemForm>
	) => {
		dispatch({
	...
	const changeSubMenuItemFieldValue = (
		fieldName: keyof SubMenuItemForm,
		value: ValueOf<SubMenuItemForm>
	) => {
		dispatch({
	...
	const moveMenuItemUp = (
		uuid: MenuItem['uuid'],
		languageCode: LanguageCode
	) => {
		dispatch({
			type: MenuStateActionType.MENU_ITEM_MOVED_UP,
			payload: { uuid, languageCode },
		});
	};
	const moveMenuItemDown = (
		uuid: MenuItem['uuid'],
		languageCode: LanguageCode
	) => {
		dispatch({
	...
	const moveSubMenuItemUp = (
		...
		dispatch({
			...
			payload: { uuid, columnIndex, subMenuItemsColumnsUuid },
		});
	};

	const moveSubMenuItemDown = (
		uuid: SubMenuItem['uuid'],
		columnIndex: number,
		subMenuItemsColumnsUuid: keyof SubMenuItemsColumns
	) => {
