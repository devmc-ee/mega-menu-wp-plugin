...
import { LocalMenuUpdateAction, MenuItemForm, MenuState, MenuStateActionType } from '../src/reducers/menuState.types';
...
describe('menuStateReducer', () => {
  ...
  describe(`${MenuStateActionType.MENU_ITEM_FORM_INITED} - ${MenuStateActionType.MENU_ITEM_FORM_REMOVED}`, () => {
    it('should return state with default menuItemForm values', () => {
      const state1 = menuStateReducer(initialState, {
        type: MenuStateActionType.MENU_ITEM_FORM_INITED, payload: {
          languageCode: 'en'
        }
      });
      expect(state1.menuItemForm).toEqual(menuItemFormInitState);

      const state2 = menuStateReducer(state1, { type: MenuStateActionType.MENU_ITEM_FORM_REMOVED });
  ...
  describe(`${MenuStateActionType.MENU_ITEM_FORM_INITED_FOR_EDITING}`, () => {
    it('should return state with default menuItemForm values', () => {
      ...
      const state1 = menuStateReducer(initState, {
        type: MenuStateActionType.MENU_ITEM_FORM_INITED_FOR_EDITING, payload: {
          languageCode: 'en',
          uuid: uuid
        }
      });
  ...
  describe(MenuStateActionType.MENU_ITEM_FORM_FIELD_CHANGED, () => {
    it('should return state with updated menuItemForm values', () => {
      const state1 = menuStateReducer({ ...initialState, menuItemForm: menuItemFormInitState },
        {
  ...
  describe(MenuStateActionType.MENU_ITEM_UUID_ADDED_TO_LOCAL_MENU, () => {
    ...
    const getLocalMenuUpdateAction = (uuid: UUID): LocalMenuUpdateAction => ({
    ...
    it('should return updated state of the localMenu', () => {
      const state1 = menuStateReducer(initState, getLocalMenuUpdateAction(uuid1));

      expect(state1.localMenu).toEqual({
        en: [uuid1]
      });

      const state2 = menuStateReducer(state1, getLocalMenuUpdateAction(uuid2));
  ...
  describe(MenuStateActionType.MENU_ITEM_UUID_DELETED_FROM_LOCAL_MENU, () => {
    const uuid1 = randomUUID();
    const uuid2 = randomUUID();
    ...
    const getLocalMenuDeleteAction = (uuid: UUID): LocalMenuUpdateAction => ({
    ...
    it('should return updated state of the localMenu', () => {
      const state1 = menuStateReducer(initState, getLocalMenuDeleteAction(uuid1));

      expect(state1.localMenu).toEqual({
        en: [uuid2]
      });

      const state2 = menuStateReducer(state1, getLocalMenuDeleteAction(uuid2));
  ...
  describe(MenuStateActionType.MENU_ITEM_SAVED, () => {
    ...
    it('should save menu item from form to localMenu & menuItems ', () => {
      ...
      const { isSubmitted, ...menuItemForm } = state1.menuItemForm as MenuItemForm;
  ...
  describe(MenuStateActionType.SUB_MENU_ITEMS_COLUMNS_INITED, () => {
    ...
    it('should add subMenuItemColumnsUuid to menuItemForm and subMenuItemsColumns', () => {
      ...
      expect(state.menuItemForm?.subMenuItemsColumnsUuid).toBe(columnsSetUuid);
  ...
  describe(MenuStateActionType.SUB_MENU_ITEM_FORM_FIELD_CHANGED, () => {
    ...
    it('should change value of subMenuItemForm', () => {
      ...
      expect(state.subMenuItemForm?.url).toBe('https://localhost.com/sub-item');
    })
  });
  ...
  describe(MenuStateActionType.SUB_MENU_ITEM_FORM_SAVED, () => {
    ...
    it('should save new subMenuItem', () => {
      ...
      expect(state1.menuItems[menuItemUuid].subMenuItemsColumnsUuid).toBe(columnsSetUuid);
      const { uuid: subMenuItemUuid, ...subMenuItem } = Object.values(state1.subMenuItems)[0];
      ...
      expect(state1.subMenuItemsColumns[columnsSetUuid][1]).toContain(subMenuItemUuid);
    });
    ...
    it('should save edited subMenuItem', () => {
      ...
      expect(state1.subMenuItemForm?.isSubmitted).toBeTruthy();
      expect(state1.subMenuItemsColumns[columnsSetUuid][1].length).toBe(1);
    })
  });
  ...
  describe(MenuStateActionType.SUB_MENU_ITEM_REMOVED, () => {
    ...
    it('should remove subMenuItem', () => {
      ...
      const { subMenuItemForm, subMenuItems, menuItemForm, subMenuItemsColumns } = state;
      
      expect(subMenuItemForm!.isSubmitted).toBeTruthy();
      expect(subMenuItemUuid in subMenuItems).toBeFalsy();
      expect(subMenuItemsColumns[menuItemForm!.subMenuItemsColumnsUuid!][subMenuItemForm!.columnIndex]).not.toContain(subMenuItemUuid);
    });
