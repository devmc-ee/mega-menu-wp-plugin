...
describe('useApiRequest', () => {
  ...
  it('Should send request with the menu state data', async () => {
    //@ts-ignore
    const wrapper = ({ children }) => (<MenuDataContextProvider><ApiServiceProvider>{children}</ApiServiceProvider></MenuDataContextProvider>)

    const { result: useMenuResult } = renderHook(() => useMenuState(), { wrapper });
    act(() => {
      useMenuResult.current.dispatchers.initMenuItemForm('en');
      useMenuResult.current.dispatchers.changeMenuItemFieldValue('title', 'test title');
      useMenuResult.current.dispatchers.changeMenuItemFieldValue('url', '/test-url');
    ...
    expect(fetch).toBeCalledWith('', {
      body: JSON.stringify({
        data: {
          ...
          subMenuItemsColumns: useMenuResult.current.state.subMenuItemsColumns
        },
        postID: 0
      }),
      headers: {
